...
class DataType {
  public:
    ...
    // Reads `data` from `bytes` with the `offset`. If the `bytes` is not large
    // enough, returns Error.
    virtual Result Read(DataItem &item, const std::vector<uint8_t> &bytes,
                        const size_t offset) const = 0;

    // Writes `data` to `bytes` with the `offset`. If the `bytes` is not
    // large enough, returns Error.
    virtual Result Write(const DataItem &item, std::vector<uint8_t> &bytes,
                         const size_t offset) const = 0;
};
