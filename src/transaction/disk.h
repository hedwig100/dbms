...
class Block {
  public:
    ...
    // Writes the `item` of type `type` with the `offset`.
    Result Write(const int offset, const data::DataType &type,
                 const data::DataItem &item);

    Result Read(const int offset, const data::DataType &type,
