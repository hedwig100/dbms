...
Result Transaction::Write(const disk::DiskPosition &position,
    ...
    Result previous_data = block.ReadBytes(
        position.Offset(), type.ValueLength(), previous_item_bytes);
    ...
    ResultV<dblog::LogSequenceNumber> lsn_result =
        recovery_manager_.WriteLog(dblog::LogOperation(
            transaction_id_, position, type, previous_item_bytes, item));
    ...
    Result write_result = block.Write(position.Offset(), type, item);
...
Result Transaction::Read(const disk::DiskPosition &position,
                         const data::DataType &type, data::DataItem &item) {
    ...
    read_result = block.Read(position.Offset(), type, item);
    if (read_result.IsError()) {
        ROLLBACK(read_result);
        return read_result + Error("transaction::Transaction::ReadByte() "
...
ResultV<uint8_t> Transaction::ReadByte(const disk::DiskPosition &position) {
    data::DataItem item;
    FIRST_TRY(Read(position, data::kTypeByte, item));
    TRY_VALUE(byte_result, data::ReadByte(item));
    return byte_result;
}

ResultV<int> Transaction::ReadInt(const disk::DiskPosition &position) {
    data::DataItem item;
    FIRST_TRY(Read(position, data::kTypeInt, item));
    TRY_VALUE(int_result, data::ReadInt(item));
    return int_result;
}

ResultV<std::string> Transaction::ReadString(const disk::DiskPosition &position,
                                             const size_t length) {
    data::DataItem item;
    FIRST_TRY(Read(position, data::TypeChar(length), item));
    TRY_VALUE(str_result, data::ReadString(item, length));
    return str_result;
}
